...
// ── 1. 纯函数：日期解析 ────────────────────────────────────────────────────
{
	...
	// 裸 M/D 过期日期滚动到下一年
	assert.deepEqual(parseAnchorDates("8/12 已过期", today).map(iso), ["2027-08-12"]);
	// 中文日期
...
// ── 2. 纯函数：扫描与渲染 ──────────────────────────────────────────────────
{
	...
	try {
		...
		assert.equal(hits.length, 2, "只应扫到 due + near，模板/示例/索引跳过");
		assert.equal(hits.filter((h) => h.kind === "due").length, 1);
...
// ── 5. 插件装配：tushare 命令 ───────────────────────────────────────────────
{
	...
	try {
		// 格式校验
		const bad = await tushare.handler({ rawInput: "set not-a-token", agent });
		assert.equal(bad.kind, "error");
		assert.ok(bad.text.includes("格式不对"));
		...
		assert.ok(saved.text.includes("在线校验通过"), saved.text);
		...
		assert.equal(warned.kind, "success");
		assert.ok(warned.text.includes("在线校验未通过"), warned.text);
