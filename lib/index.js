			...
			try {
				switch (action) {
					case "set": {
						...
						// 不校验长度/格式：原样保存（仅去掉不可见零宽字符），然后自动在线校验
						...
						if (check.code === "NETWORK") {
							return { kind: "success", text: `Tushare token 已保存（${suffix}）${note}，当前网络不可用未能在线校验——可稍后 /resanity-tushare test\n凭据文件：${path}` };
						}
						return {
							kind: "error",
							text: `Tushare token 已保存（${suffix}）${note}，但自动在线校验未通过（code ${check.code}${check.msg ? `：${check.msg}` : ""}）——token 可能无效，请检查后重新 /resanity-tushare set\n凭据文件：${path}`,
						};
					}
					case "status": {
