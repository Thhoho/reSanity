...
当前源码树只保留 core contract、investing profile、open network、anchor、trigger、install identity 和同 hash A/B 的可复用验证协议；候选过程记录和旧协议留在 Git 历史，不进入 0.2.1 发布树。
目录名与 JSON 中的 `v1` / `v2` 是机械数据合同的 schema 代际，不是 Resanity 产品版本。
